# ============================================================== #
# Script to Run all of Gibbs Sampling for Different experiments  #
# ============================================================== #

rm(list = ls())

setwd("~/Documents/Research/Density_Project/data_aug_reject_smpl/flow_cyt/update_functions_v4")
...
source("./niw.R")
source("./post_pred_func.R")
source("./MoG.R")
source("./impute_func.R")
source("./import_sampling.R")
source("./dtmvnorm_fast.R")
...
# Use type = 1 for one dimensional beta/gaussian observations
# Use type = 2 for flowcytometry data
# Use type = 3 for crime data
# Use type = 4 for bivariate gaussian
type <- 2

# Type of model: motgt, motg, tmogt
mix  <- "tmogt"

if (type == 1){
  data_type = "gauss"

  # Number of mode: bimodal, unimodal
  # only for Gauss
  mode_num <- "unimodal"
  ...
  stop("type is less than 1 or greater than 4")
}

if (type == 3){
  # Sampling method: package, is
  samp_method <- "is"
} else {
  samp_method <- "package"
}
...
if (type == 1){
  # Running for 1 dimensional Gauss and Beta
...
if (type == 2){
  ...
  # # Prior parameters
  # mu_pr <- max(rowMeans(ip_data))
  # sd_pr <- max(apply(ip_data, 1, sd))*3

  # bound_data    <- (ip_data - rep(0.3, 4))*1.1 + rep(0.3, 4)
  # bound_data_cs <- constr(bound_data)
  ...
  # df_new   <- data.frame(t(ip_data_ctrl), bound_data_cs)
  #
  # plt <- ggplot(df_new) + geom_point(aes(x = CD4, y = CD8b, colour = bound_data), size = 0.5)
  # plt
