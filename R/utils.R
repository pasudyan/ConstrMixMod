...
#' @param dataType one of "gauss", "beta", "flowcyto", "crime", "bvgauss"
...
#' @export
#'
gibbSamplingOri <- function(K, ipdata, thr = Inf, burnIn, numIter, constr, mix, lower, upper, dataType) {
  ...
  if (dm == 1){
    ...
    if (dataType == "flowcyto"){
      niwpr  <- list(mu0 = rep(1/2, dm), lam = 1e-2,
                     Phi = diag(rep(1e-3, dm)),  nu = dm+1)
    } else if (dataType == "bvgauss"){
      niwpr  <- list(mu0 = 1/2, lam = 1/10,
                     Phi = diag(rep(1e-3, dm)),  nu = dm+1)
  ...
  params  <- updateParams(K, alp, ipdata, dm, niwpr, z)
  ...
  for(iter in 1:numIter) {
    ...
    if (mix == "motg" | mix == "motgt"){
      ...
      for(i in 1:K) {
        pm <- updateParamsComponent(X = ipdata[, z == i], mu = params$mu[[i]],
                                    cv = params$cv[[i]], niwpr, constr, thr)
      ...
      # Update weights
      params$wt <- updateWeights(z, K, alp)
      ...
      # Sample rejections
      smplsRejs <- imputeRejections(K, params, N, constr, thr, ceilRej)

      # Function to update params
      params <- updateParamsTmog(K, alp, xRejs = cbind(ipdata, smplsRejs$rejs),
                                 params$wt, niwpr, zRejs = c(z, smplsRejs$z))
      # Update weights
      params$wt <- updateWeights(c(z, smplsRejs$z), K, alp)

      # Count number of rejections in partition
      if (iter > burnIn)
        rslt[[iter - burnIn]]$rejSamp <- smplsRejs$rejs

      # Update assignments
      z <- updateClustAssignTmog(K, ipdata, N, dm, params)

      if (iter > burnIn){
        rslt[[iter - burnIn]] <- params
        rslt[[iter - burnIn]]$cntRej  <- smplsRejs$rej_acc
      }
    }
